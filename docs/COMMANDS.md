...
cross-schema references, closed root and nested objects, schema-version
constants, replay/test status enums, required keys, non-blank required identity
strings, unique set-like arrays, non-blank file-list entries, numeric range
bounds, RFC3339 timestamps, eval-summary totals, and timestamp ordering. It can
also validate explicit JSON/schema pairs.
